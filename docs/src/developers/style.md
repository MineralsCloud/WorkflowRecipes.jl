...
!!! info
    The style guide is always a work in progress, and not all WorkflowRecipes code
    follows the rules. When modifying WorkflowRecipes, please fix the style violations
...
`WorkflowRecipes` uses [`JuliaFormatter`](https://github.com/domluna/JuliaFormatter.jl) as
an auto-formatting tool.

We use the options contained in [`.JuliaFormatter.toml`](https://github.com/MineralsCloud/WorkflowRecipes.jl/blob/main/.JuliaFormatter.toml).

To format your code, `cd` to the `WorkflowRecipes` directory, then run:
...
!!! info
    A continuous integration check verifies that all PRs made to `WorkflowRecipes` have
