...
[`WorkflowRecipes`](https://github.com/MineralsCloud/WorkflowRecipes.jl).
...
2. Run the following commands and wait for them to finish:
   ...
   julia> Pkg.add("WorkflowRecipes")
   ```

3. Run

   ```julia-repl
   julia> using WorkflowRecipes
   ```
...
version of `WorkflowRecipes`, type

```@repl
using Pkg
Pkg.update()
pkg"add https://github.com/MineralsCloud/WorkflowRecipes.jl"
```

in the second step above.

## Update `WorkflowRecipes`
...
our [GitHub repository](https://github.com/MineralsCloud/WorkflowRecipes.jl)
for new releases.
Once we release a new version, you can update `WorkflowRecipes` by typing

```@repl
using Pkg
Pkg.update("WorkflowRecipes")
Pkg.gc()
```

in the Julia REPL.

## Uninstall and reinstall `WorkflowRecipes`
...
1. To uninstall, in a Julia session, run
   ...
   julia> Pkg.rm("WorkflowRecipes")

   julia> Pkg.gc()
   ```

2. Press `ctrl+d` to quit the current session. Start a new Julia session and
   reinstall `WorkflowRecipes`.
